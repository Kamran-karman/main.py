# ver = 0.2.3.1
# Была исправлена ошибка с IndexError: list index out of range. Теперь её нет.
import random
import time
...
DIALOG1 = ['Кто ты, странник?', 'Я Войслав, воин СЗА. А Вы кто?', 'Я Бренд, здешний командир ополченцев. Мы охраняем '
           ...
           'не дам, а от вдруг по пути к убежищу предателей, ты и мой меч потеряешь. ', 'Ха-ха-ха', 'Поэтому, Войслав я'
...
class IgraPrologViev(arcade.View):
    ...
    def stan(self, sprite, s, vr=120):
        if s <= vr:
            sprite.change_x = 0
            sprite.change_y = 0

    def oglush(self, sprite, s, center_y, vr=120):
        c_y = None
        if s == 0:
            c_y = center_y
        if s < vr:
    ...
    def on_update(self, delta_time: float):
        ...
        if arcade.check_for_collision_with_list(self.igrok, self.hill_zele_list):
