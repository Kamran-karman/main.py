...
class Vrag(arcade.Sprite):
    def __init__(self, igrok, sprite_list, v_drug_list, kast_scena=False, vr_stan=150, vr_oglush=150, drav=False):
        ...
        self.mech = sposob.Mech(self, self.igrok_list, None, self.storona, 20)
                ...
                for drug in self.v_drug_list:
                    if self.s <= 180:
                        if (drug.center_x > self.center_x and abs(self.right - drug.left) <= 50 and not drug.go
                            ...
                            self.force_x = 0.
                            break
                    elif (self.s > 180 and not self.go and arcade.check_for_collision(self.radius_prig, drug) and
                          (drug.center_x > self.center_x or drug.center_x < self.center_x) and
                          (self.igrok.center_x > self.center_x or self.igrok.center_x < self.center_x)):
                        self.go = False
                        if self.igrok.center_x > self.radius_vid.center_x:
                            if abs(self.left - self.igrok.right) > self.d_zone:
                                self.force_x = 15000.
                            elif abs(self.left - self.igrok.right) <= self.d_zone:
                                self.force_x = 0.
                                self.go = False
                                self.s = 0
                                break
                            ...
                            if abs(self.right - self.igrok.left) > self.d_zone:
                                self.force_x = -15000.
                            elif abs(self.right - self.igrok.left) <= self.d_zone:
                                self.force_x = 0.
                                self.go = False
                                self.s = 0
                                break
                        ...
                        if (arcade.check_for_collision_with_list(self.radius_prig, self.sprite_list) and
                                self.is_on_ground and abs(dx) < D_ZONE):
                            if arcade.check_for_collision(self.radius_prig, drug):
                                self.force_y = 50000.
                            else:
                                self.force_y = 0.
            else:
